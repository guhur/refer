...
class RefEvaluation:
        ...
        for scorer, method in scorers:
            ...
            if type(method) == list:
                for sc, scs, m in zip(score, scores, method):
                    self.setEval(sc, m)
                    self.setRefToEvalRefs(scs, list(self.refToGts.keys()), m)
                    print("%s: %0.3f" % (m, sc))
            else:
                self.setEval(score, method)
                self.setRefToEvalRefs(scores, list(self.refToGts.keys()), method)
    ...
    def setEvalRefs(self):
        self.evalRefs = [eval for refId, eval in list(self.refToEval.items())]
...
if __name__ == "__main__":
    ...
    for metric, score in list(refEval.eval.items()):
